...
void FormulaAutonomousSystem::generateGlobalPath() {
    ...
    for (double s = 0; s < total_length; s += 0.5) { // Sample every 0.5m
        ...
        // Signed Curvature (Kappa): 양수면 좌회전, 음수면 우회전 (중앙선 기준)
        double curvature = (dx * ddy - dy * ddx) / std::pow(dx * dx + dy * dy, 1.5);

        // 3. [Out-In-Out 로직] 오프셋 계산 및 적용
        double abs_curvature = std::abs(curvature);
        
        // 1. 곡률에 비례하는 오프셋 크기 계산
        double desired_offset_mag = abs_curvature * params.curvature_offset_gain_;
        
        // 2. 최대 허용 오프셋 크기 제한 (안전 장치)
        double max_allowed_offset = params.max_lane_width_est_ * params.max_offset_ratio_;
        double final_offset_mag = std::min(desired_offset_mag, max_allowed_offset);

        // 3. 최종 오프셋 방향 결정:
        // Curvature > 0 (좌회전) -> 오른쪽(In)으로 오프셋 -> signed_offset은 음수
        // Curvature < 0 (우회전) -> 왼쪽(In)으로 오프셋 -> signed_offset은 양수
        double signed_offset = (curvature > 0) ? -final_offset_mag : final_offset_mag;
        
        // 경로의 왼쪽을 가리키는 수직 벡터 (Normal Vector)
        double nx_left = -std::sin(yaw);
        double ny_left = std::cos(yaw);
        
        // 중앙점에 오프셋 적용
        double shifted_x = x + signed_offset * nx_left;
        double shifted_y = y + signed_offset * ny_left;

        // 이동된 점을 임시 경로에 저장합니다. (속도 계산을 위해 곡률은 중앙선 기준으로 유지)
        temp_path.emplace_back(shifted_x, shifted_y, yaw, curvature, 0.0, s); 
    }
